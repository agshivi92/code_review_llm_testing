...
CREATE OR REPLACE PROCEDURE KIPI.ADMIN.INFERENCING_SP_1("TGT_DB_NAME" VARCHAR(16777216), "TGT_SCHEMA_NAME" VARCHAR(16777216), "TGT_TABLE_NAME" VARCHAR(16777216))
...
def main(session: snowpark.Session, tgt_db_name: str, tgt_schema_name: str, tgt_table_name: str):
    ...
    try:
        ############ LOADING THE FIRST CHUNK OF 35M DATASET##################
        test_table = f"{tgt_db_name}.{tgt_schema_name}.TEST_35M"
        ...
        pred1 = predict_model(top_models,data = df,raw_score=True)
        ...
        sf_df.write.mode(''overwrite'').save_as_table(f"{tgt_db_name}.{tgt_schema_name}.{tgt_table_name}")
...
call INFERENCING_SP_1('KIPI','ADMIN','PRED_FINAL');